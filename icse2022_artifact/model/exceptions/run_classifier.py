...
def evaluate(args, model, tokenizer, checkpoint=None, prefix="", mode='dev'):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        print("recall", recall)
        print("F1 sanity check", 2 * (precision * recall) / (precision + recall))

        print("TRUE POSITIVES", tp)
        print("FALSE POSITIVES", fp)
        if tp + fp:
            print("POS acc", pos_cor / (tp + fp))
        if tn + fn:
            print("NEG acc", neg_cor / (tn + fn))
        ...
        if (mode == 'dev'):
            ...
            result_df.to_csv('exception_preds.csv', index=False)

            # with open("log_valid.txt", "w") as f:
            ...
            for i, p_label in tqdm(enumerate(preds_label), desc='Testing'):
                print("true label", examples[i].label, "pred label", p_label, "test", examples[i].text_b, "socres", all_logits[i])
