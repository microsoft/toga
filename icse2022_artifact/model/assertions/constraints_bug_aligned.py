...
if __name__=='__main__':
    ...
    # '''
    bug_finding_asserts = []
    with open("bug_finding_asserts.txt") as f:
        reader = csv.reader(f) 
        for row in reader:
    ...
    test_meta_dict = {}
    for bug_meta, line in zip(bug_finding_asserts, lines):
        if not line: continue

        _, project, bug, test_id, assert_error, test_full_id = bug_meta
        ...
        test_meta_dict[test_num] = (project, bug, test_id, assert_error, test_full_id)
    ...
    for test_num, values in foo.items():
        ...
        # if p_1s[0] == 0:
        project, bug, test_id, assert_error, test_full_id = test_meta_dict[test_num]
        if correct:
            found_bugs.add(project+bug)
        ...
        print(project, bug, test_full_id)
        print(test)
        print(fm)
    ...
    print("num correct", cor, "num incorrect", incor, cor+incor)
    print('found bugs:', len(found_bugs), found_bugs)
