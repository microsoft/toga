...
def train(args, train_dataset, model, tokenizer, optimizer):
    ...
    for i in range(0, len(train_dataset)):
        labels.append(train_dataset[i][3].item())

    num_1s = len([l for l in labels if l])
    num_0s = len([l for l in labels if not l])

    class_sample_counts = [num_0s, num_1s]
    class_weights = 1./torch.Tensor(class_sample_counts)
    train_dataset_weights = [class_weights[class_id] for class_id in labels]

    weighted_train_sampler = WeightedRandomSampler(train_dataset_weights, len(train_dataset))

    train_dataloader = DataLoader(train_dataset, sampler=weighted_train_sampler, batch_size=args.train_batch_size)
    ...
    for idx, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            inputs = {'input_ids': batch[0],
                      ...
                      'labels': batch[3]}
            num_1s = len([l for l in batch[3] if l])
            num_0s = batch[3].shape[0] - num_1s

            #print("labels in batch {}: 0: {}, 1: {}".format(step, num_0s, num_1s))
        ...
        logger.info("Training loss: " + str(tr_loss / global_step))
...
def evaluate(args, model, tokenizer, checkpoint=None, prefix="", mode='dev'):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        if (mode == 'dev'):
            eval_dataset, _ = load_and_cache_examples(args, eval_task, tokenizer, ttype='dev')
        elif (mode == 'test'):
        ...
        pos_cor, neg_cor = 0, 0
        tp, fp, tn, fn = 0, 0, 0, 0
        for t, p in list(zip(out_label_ids, preds_label)):
            if t == p:
                cor += 1
                if t:
                    pos_cor += 1
        ...
        print("TOTAL", len(out_label_ids), len(preds_label))
        print("TRUE POSITIVES", tp)
        print("FALSE POSITIVES", fp)
        print("TRUE NEGATIVES", tn)
        print("FALSE NEGATIVES", fn)
        #print("POS acc", pos_cor / len([t for t in out_label_ids if t]))
        print("NEG acc", neg_cor / len([t for t in out_label_ids if not t]))

        #print(list(zip(out_label_ids, preds_label)))
        ...
        if (mode == 'dev'):
            ...
            l1 = [str(l[0]) for l in all_logits]
            l2 = [str(l[1]) for l in all_logits]
            ...
            for i in range(len(preds_label)):
                # if not examples[i].text_a == prev_test:
                    # test_count += 1

                # test_counts += [test_count]
                idxs += [examples[i].guid] # guid is test idx
                prev_test = examples[i].text_a

                m = test_name_re.match(examples[i].text_a)
                if not m:
                    g = "test-name-placeholder"
                else:
                    g = m.group(1)
                test_names += [g]

                assertions += [examples[i].text_c]
                tests += [examples[i].text_a]
                fms += [examples[i].text_b]

                truncations += [examples[i].trunc]

            result_df = pd.DataFrame({
                'idxs': idxs, 
                'true_lbl': out_label_ids,
                'pred_lbl': preds_label,
                'logit_0': l1,
                'logit_1': l2, 
                'test_name': test_names,
                'pred_assertion': assertions,
                'truncation': truncations,
                'test': tests,
                'fms': fms
            })

            result_df.to_csv('assertion_preds.csv', index=False)

            '''
            with open("log.txt", "w") as f:
                f.write("\n".join([",".join(x) for x in zip([str(t) for t in out_label_ids], [str(p) for p in preds_label], l1, l2)]))
            '''
            

            if PRINT_ALL_PREDS:
                prev_test = None
                print(len(examples), len(preds_label))
                for i, p_label in tqdm(enumerate(preds_label), desc='Testing'):
                    if not examples[i].text_a == prev_test:
                        print( "test", examples[i].text_a, "method", examples[i].text_b)
                    print("pred label", p_label, "true label", examples[i].label, "assertion", examples[i].text_c, "socres", all_logits[i])
                    print("-"*100)
                    print()
                    prev_test = examples[i].text_a
...
def main():
    ...
    # Training
    if args.do_train:
        train_dataset, _ = load_and_cache_examples(args, args.task_name, tokenizer, ttype='train')
